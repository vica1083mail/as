...
export const QuizLevel3 = ({
  ...
  const washedacar = useRef();
  const styledaroom = useRef();
  const progrannedaform = useRef();
  ...
  const applyFormData = (e) => {
    ...
    sendPhone("+375" + phone.current.value);
    sendCookingDone(cookingRef.current.checked);
    sendWashingDone(washedacar.current.checked);
    sendStylingDone(styledaroom.current.checked);
    sendProgrammingDone(progrannedaform.current.checked);
  };
  ...
  return (
    <form className="document-input" onSubmit={(e) => applyFormData(e)}>
      ...
      <input id="phone-input" placeholder="44 123 45 67" ref={phone} />
      ...
      <div id="activities-input">
        ...
        <input type="checkbox" id="washing" ref={washedacar} />
        <label htmlFor="washing">Washed a car</label>
        <br />

        <input type="checkbox" id="styling" ref={styledaroom} />
        <label htmlFor="styling">Styled a room</label>
        <br />

        <input type="checkbox" id="programming" ref={progrannedaform} />
